...
const LegalPageLayout = ({ title, summary, children }: LegalPageLayoutProps) => {
  ...
  const legalNavigation = [
    { label: messages.legalLayout.navigation.terms, href: LEGAL_LINKS.terms },
    { label: messages.legalLayout.navigation.privacy, href: LEGAL_LINKS.privacy },
    { label: messages.legalLayout.navigation.disclaimer, href: LEGAL_LINKS.disclaimer },
    { label: messages.legalLayout.navigation.imprint, href: LEGAL_LINKS.imprint },
  ];
      ...
      <main className="flex-1 pt-16">
        <div className="container py-12 md:py-16">
          <Link to="/" className="mb-6 inline-flex items-center gap-2 text-sm font-medium text-muted-foreground transition-colors hover:text-foreground">
            ...
            {messages.legalLayout.back}
          </Link>

          <div className="rounded-[28px] bg-card p-6 shadow-card md:p-10">
            <div className="mb-8 space-y-4 border-b border-border pb-8">
              <p className="text-sm font-medium uppercase tracking-[0.2em] text-primary">{messages.legalLayout.title}</p>
              ...
              <p className="text-xs uppercase tracking-[0.16em] text-muted-foreground">
                {messages.legalLayout.lastUpdated}: {messages.legal.lastUpdatedDate}
              </p>
            </div>

            <div className="grid gap-10 lg:grid-cols-[240px_minmax(0,1fr)]">
              <aside className="h-fit rounded-2xl border border-border bg-secondary/70 p-4">
                <p className="mb-3 text-sm font-semibold text-foreground">{messages.legalLayout.documents}</p>
